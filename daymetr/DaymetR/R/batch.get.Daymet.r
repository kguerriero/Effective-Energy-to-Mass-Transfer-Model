...
#’ @export
#' @examples
#' batch.get.Daymet("mylocation.csv",start_yr=1980,end_yr=2000,internal=TRUE)

batch.get.Daymet <- function(file_location,
                                  start_yr=1980,
  ...
  for (i in 1:dim(locations)[1]){
    ...
    try(get.Daymet(site=site,lat=lat,lon=lon,start_yr=start_yr,end_yr=end_yr,internal=internal),silent=FALSE)
  }
}