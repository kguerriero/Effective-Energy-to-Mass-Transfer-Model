...
#' @examples
#' get.Daymet("testsite",lat=36.0133,lon=-84.2625,start_yr=1980,end_yr=2000)
#’ @export

get.Daymet <- function(site="Daymet",
                            lat=36.0133,
                            lon=-84.2625,
                            start_yr=1980,
  ...
  download.file(download_string,daymet_file,quiet=TRUE)
  
  # test for out of spatial range values (empty download file, 0 bytes)
  if (file.info(daymet_file)$size == 0 ){
    # if data file has a size of 0, delete file
    system(paste('rm',daymet_file,sep=' '),intern=FALSE)
